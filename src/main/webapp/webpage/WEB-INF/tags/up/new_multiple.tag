...
<%@ attribute name="files" type="java.lang.String" required="false" description="逗号分割的URL地址"%>
...
<div class="lightBoxGallery">
		...
		<input id="uploadFile${name}1" type="file" onchange="upload${name}.showImg(event)"  name="${name}"  accept="image/gif,image/jpeg,image/jpg,image/png,image/svg"   
		    style="height:0.5px; width:0px;border:0px; "   />
		<div id="filecontent${name}">
			 
            
           <c:forEach items="${fns:split2Map(files)}" var="file">
				<div class="img-div">
					<p class="img-p" style="right:3px!important" onclick="upload${name}.delImg(this,'${file.key}')"  imagetype='1'   ></p>  <!-- 1为回显  2为动态添加 -->
	                <img src="${file.key}" style="height: 100px;width: 100px;">
				</div>
            </c:forEach>
		</div>
</div>
