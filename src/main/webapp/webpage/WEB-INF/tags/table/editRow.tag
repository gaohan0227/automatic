...
debugger;
		  ...
		  if(size > 1 ){
				top.layer.alert('<spring:message code="global.edit.onlyone"/>', {icon: 0, title:'<spring:message code="global.warn"/>'});
				return;
			  }
		    var id =  $("#${id} tbody tr td input.i-checks:checkbox:checked").attr("id");
		    var tag="${target}";
			if("true"=="${needTarget}"){
				tag=findCurrPopDialogIframeId();
			}
			alert(tag);
		    openDialog("<spring:message code='global.edit'/>"+'${title}',"${url}?id="+id,"${width==null?'800px':width}", "${height==null?'500px':height}",tag);
		}
</script>    