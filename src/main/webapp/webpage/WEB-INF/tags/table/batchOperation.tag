...
<%@ attribute name="label" type="java.lang.String" required="false"%><%-- 按钮标题 --%>
<%@ attribute name="confirmTip" type="java.lang.String" required="false"%><%-- 确认提示信息 --%>
<button class="btn btn-white btn-sm" onclick="batchOper()" data-toggle="tooltip" data-placement="top"><i class="fa fa-trash-o"> ${label==null?'批量处理':label}</i>
                        </button>
	...
	function batchOper(){
		...
		var url ="${url}";
		  ...
		  if(ids == ""){
			...
			top.layer.confirm('确认要批量${confirmTip}吗?', {icon: 3, title:'系统提示'}, function(index){
