...
<%@ attribute name="hzl" type="java.lang.String" required="false" description="名称"%>
...
<script type="text/javascript">
	...
	function dealStr2(str){
		if(str == null || str == ""){
			return "";
		}
		var arr = str.split("/");
		arr.shift();
		var exchange = arr.shift();
		return "/"+exchange;
	}
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li><img src=\""+url3+urls[i]+"\" url=\""+url3+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
				li = "<li><a href=\""+url3+urls[i]+"\" url=\""+url3+urls[i]+"\" target=\"_blank\">"+decodeURIComponent((url3+urls[i]).substring((url3+urls[i]).lastIndexOf("/")+1))+"</a>";//</c:if>
