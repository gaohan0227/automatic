...
<button class="btn btn-white btn-sm" onclick="updateStatusAll(${label=='上架'?'1':'0'})" data-toggle="tooltip" data-placement="top"><i class="fa ${label=='上架'?'fa-arrow-circle-o-up':'fa-arrow-circle-o-down'} "> ${label==null?'下架':label}</i>
                        </button>
...
$(document).ready(function() {
	...
	function updateStatusAll(statusType){
		...
		var str="";
		  ...
		  if(statusType == "1"){
			...
			window.location = "${url}?ids="+ids+"&status="+statusType;
