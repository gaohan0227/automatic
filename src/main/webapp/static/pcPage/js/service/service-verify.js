...
var verify = [
  ...
  {
    type: '_realName', //真实姓名的正则验证(汉字和字母)
    regular: /^[a-zA-Z\u4e00-\u9fa5]+$/,
  },
  {
    type: 'chinese', //真实姓名的正则验证(汉字和字母)
    regular: /^[\u4e00-\u9fa5]+$/,
  },
  {
    type: '_code', //验证码
...
function fnVerify(fromDiv) {
  ...
  for (var j = 0; j < _input.length; j++) {
    ...
    if (_input[j].dataset.verify == 'true') {
      //true为验证，false为不验证
  ...
  if (_fromJson && _fromJson.length > 0) {
    for (var j = 0; j < _fromJson.length; j++) {
      if (_fromJson[j].pValue == '') {
      ...
      } else if (_fromJson[j].pValue.replace(/(^\s*)|(\s*$)/g, '') == '') {
        if (_fromJson[j].isNull) {
          //是否验证非空
          document.getElementById(_fromJson[j].pId).value = '';
          var msg = '您没有输入' + _fromJson[j].pName + '，请重新输入';
          fnToast(msg);
          return;
        }
      } else {
        if (_fromJson[j].isReg) {
          //是否验证正则
          var state = fnRegularVerify(
            _fromJson[j].pType,
            _fromJson[j].pValue,
            _fromJson[j].pName
          );
        ...
        if (_fromJson[j].isLength) {
          if (
            _fromJson[j].pValue.length < _fromJson[j].isLength[0] ||
            _fromJson[j].pValue.length > _fromJson[j].isLength[1]
          ) {
            var msg =
              _fromJson[j].pName +
              '的长度应限制在' +
              _fromJson[j].isLength[0] +
              '-' +
              _fromJson[j].isLength[1] +
              '之间';
            fnToast(msg);
            return;
          }
  ...
  if (_verifyType == 'true') {
    ...
    if (_verifyCard == 'true') {
      ...
      var birthday =
        inputVal.substr(6, 4) +
        '-' +
        Number(inputVal.substr(10, 2)) +
        '-' +
        Number(inputVal.substr(12, 2));
...
function fnToast(msg, date, loca) {
  ...
  toastDom.setAttribute('class', 'toast_data');
...
function fnRemoveToast() {
  var paras = document.getElementsByClassName('toast_data');
...
function fnOpenInputAlert(text,way) {
    fnCloseInputAlert();
    var alertDom = document.createElement('div');
    alertDom.setAttribute('class', 'alert_data');
    var alertData =
        '<div class="alert_content" id="alert_input">' +
            '<p class="alert_title">新增' +text +'</p>' +
            '<input class="alert_input" type="text" maxlength="60" placeholder="请输入'+text+'的序列号" data-verify="true" data-null="true" data-regular="false" name="序列号"/>'+
            '<div class="alert_bottom">' +
                '<p onclick="fnCloseInputAlert()">取消</p>' +
                '<p class="submit_btn" onclick="'+way+'()">确定</p>' +
            '</div>' +
        '</div>';
    alertDom.innerHTML = alertData;
    document.body.appendChild(alertDom);
}
function fnCloseInputAlert() {
    var paras = document.getElementsByClassName('alert_data');
    for (i = 0; i < paras.length; i++) {
      if (paras[i] != null) {
        paras[i].parentNode.removeChild(paras[i]);
      }
  ...
  function fnOpenAlert(text,way) {
    fnCloseAlert();
    var alertDom = document.createElement('div');
    alertDom.setAttribute('class', 'alert_data');
    var alertData =
        '<div class="alert_content">' +
            '<p class="alert_title" style="padding:50px 0;">' +text +'</p>' +
            '<div class="alert_bottom">' +
                '<p onclick="fnCloseAlert()">取消</p>' +
                '<p class="submit_btn" onclick="'+way+'">确定</p>' +
            '</div>' +
        '</div>';
    alertDom.innerHTML = alertData;
