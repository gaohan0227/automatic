...
function menuItem() {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="J_iframe" onload="loadFrame(this)" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
...
//打开选项卡菜单
function openOneTab(url,title, isNew){//isNew 为true时，打开一个新的选项卡；为false时，如果选项卡不存在，打开一个新的选项卡，如果已经存在，使已经存在的选项卡变为活跃状态。
	// 获取标识数据
	var dataUrl = url,
	dataIndex ,
	menuName = title,
...
//    });
	if(!isNew){
		top.$('.J_menuTab').each(function () {
			var tmp = dataUrl.indexOf("?") > -1 ? dataUrl.substring(0,dataUrl.indexOf("?")) : dataUrl;
			if (top.$(this).data('id').indexOf(tmp) > -1) {// 选项卡已存在，激活。
				 if (!top.$(this).hasClass('active')) {
		            	top.$(this).addClass('active').siblings('.J_menuTab').removeClass('active');
		                scrollToTab(top.$(this));
		                // 显示tab对应的内容区
		                top.$('.J_mainContent .J_iframe').each(function () {
		                    if (top.$(this).data('id').indexOf(tmp) > -1) {
		                    	top.$(this).show().siblings('.J_iframe').hide();
		                    	//显示loading提示
		                		var _loading = layer.load();
		                		top.$(this).attr("src",dataUrl).load(function () {
		                			//iframe加载完成后隐藏loading提示
